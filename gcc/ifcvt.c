  ...
  if (JUMP_P (jump))
      ...
      if (new_dest_label == NULL_RTX)
	{
	  new_dest_label = PATTERN (jump);
          if (GET_CODE (new_dest_label) == PARALLEL)
            {
              for (i = XVECLEN (new_dest_label, 0) - 1; i >= 0; i--)
                if (ANY_RETURN_P (XVECEXP (new_dest_label, 0, i)))
                  {
                    new_dest_label = XVECEXP (new_dest_label, 0, i);
                    break;
                  }
            }
