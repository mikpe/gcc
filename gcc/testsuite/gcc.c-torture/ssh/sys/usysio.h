...
**
** $Header: /home/cvsroot/ssh/tops20_h/sys/usysio.h,v 1.2 2002/12/17 23:10:28 sumit Exp $
**
*/
...
#if SYS_T20
#define UF_CH_IQ (1<<35)	/* Use sign bit to flag IQ present */
#endif
	int uf_type;		/* OS device type     (== major device #) */
#if SYS_T20
	union {
	  struct {		/* File info */
	    int dnum;		/* USYS device number (== minor device #) */
	  } file;
	  struct {		/* Socket info */
	    u_char af;		/* Address family */
	    u_char type;	/* Socket type */
	    u_char proto;	/* Protocol */
	    u_int so_opt;	/* Socket options */
	    u_int ctimeo;	/* Connection timeout */
	    u_int ttimeo;	/* Transmission timeout */
	  } sock;
	}dd;			/* Device dependent data */
/* Quick ref symbols for file type */
#define uf_dnum dd.file.dnum
/* Quick ref symbols for socket type */
#define uf_sock_af dd.sock.af
#define uf_sock_type dd.sock.type
#define uf_sock_proto dd.sock.proto
#define uf_sock_so_opt dd.sock.so_opt
#define uf_sock_ctimeo dd.sock.ctimeo
#define uf_sock_ttimeo dd.sock.ttimeo
#else
...
#if SYS_T20
	int uf_lhost;		/* Local host number */
	int uf_fhost;		/* Foreign host number */
	short uf_lport;		/* Local port number */
	short uf_fport;		/* Foreign port number */
#endif /* SYS_T20 */
...
#define UIOF_SOCKET	FDF_SOCKET	/* T20: socket, not file */
...
/* From cinet:send.c */
extern int _send P_((int fd, struct _ufile *uf, const void *buf, size_t len,
		     int flags, const struct sockaddr *to, int tolen));
/* From cinet:recv.c */
extern int _recv P_((int fd, struct _ufile *uf, void *buf, size_t len,
		     int flags, struct sockaddr *from, int *fromlen));
/* From cinet:connect.c */
extern int _iqpmx;		/* Max internet queue packet size */
extern int _asniq(int fd, struct _ufile *uf, int proto,
		  int fhost, int fport,
		  int lhost, int lport);
/* From cusys:read.c */
extern int _read (struct _ufile *uf, void *vbuf, size_t nbytes);
/* From cusys:write.c */
extern int _write (struct _ufile *uf, const void *vbuf, size_t nbytes);
#if SYS_T20+SYS_10X
