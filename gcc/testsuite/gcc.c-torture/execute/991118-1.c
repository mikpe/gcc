...
main()
{
  ...
  if (tmp.pad != 0x123 || tmp.field != 0x000001551175BDFDB5LL)
    abort ();
  if (tmp2.pad != 0x123 || tmp2.field != 0x000001551175BDFDB5LL)
    abort ();

  tmp3 = sub3 (tmp3);
  tmp4 = sub4 (tmp4);
  if (tmp3.pad != 0x123 || tmp3.field != 0xFFFFF989AB12345678LL)
    abort ();
  if (tmp4.pad != 0x123 || tmp4.field != 0xFFFFF989AB12345678LL)
    abort ();
  exit (0);
}
#else
