...
# endif
  static char const *SVNID _unused_ = "$Id: hash.c f3ded7179433 2007/04/26 21:47:09 Martin Chaney <chaney@xkl.com> $";
...
struct hash_entry *
hash_lookup (struct hash_table *table, hash_table_key key, int create, 
...
struct hash_entry *
hash_newfunc (struct hash_entry *entry, struct hash_table *table, hash_table_key p ATTRIBUTE_UNUSED)
{
  if (entry == 0)
...
void
hash_traverse (struct hash_table *table, bool (*func) PARAMS ((struct hash_entry *, hash_table_key)), PTR info)
{
  unsigned int i;
  struct hash_entry *p;
...
bool
string_compare (hash_table_key k1, hash_table_key k2)
{
...
hash_table_key
string_copy (struct obstack *memory, hash_table_key k)
{
  char *new;
  char *string = (char *) k;
